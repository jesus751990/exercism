public static class DialingCodes
{
    ...
    public static Dictionary<int, string> RemoveCountryFromDictionary(Dictionary<int, string> existingDictionary, int countryCode)
        => existingDictionary.Remove(countryCode) ? existingDictionary : existingDictionary;

    public static string FindLongestCountryName(Dictionary<int, string> existingDictionary)
        => existingDictionary.Values.OrderByDescending(name => name.Length).FirstOrDefault(string.Empty);
}